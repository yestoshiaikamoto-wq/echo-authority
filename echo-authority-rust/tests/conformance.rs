...
fn fixture() -> Fixture {
    ...
    let mut authority = AuthorityObject {
        issuer: "did:echo:human:427".into(),
        subject: "agent:booking".into(),
        ...
        issuer_pub: issuer_pub.clone(),
        subject_pub: subject_pub.clone(),
        sig: String::new(),
    };
    ...
    let state = VerifierState {
        ...
        uses: BTreeMap::new(),
        spent_total_minor: BTreeMap::new(),
        offline_limit_minor: 2_000,
        max_clock_skew_sec: 30,
        max_invocation_age_sec: 300,
        trusted_issuers,
...
fn resign_invocation(f: &mut Fixture) {
    sign_invocation(&mut f.invocation, &f.subject_key);
}

#[test]
fn valid_request_allows() {
    assert_eq!(
        border_check(
            &fixture().authority,
            &fixture().invocation,
...
fn self_asserted_issuer_key_is_rejected() {
    ...
    sign_invocation(&mut f.invocation, &f.subject_key);
    assert_eq!(
        border_check(&f.authority, &f.invocation, &f.state).reason,
...
fn value_and_total_limits_are_enforced_in_minor_units() {
    let mut f = fixture();
    f.invocation.amount_minor = 4_001;
    resign_invocation(&mut f);
    assert_eq!(
        border_check(&f.authority, &f.invocation, &f.state).reason,
        Reason::ValueLimitExceeded
    );
    f.invocation.amount_minor = 4_000;
    resign_invocation(&mut f);
    f.state
        .spent_total_minor
        .insert(f.invocation.authority_id.clone(), 18_000);
    assert_eq!(
        border_check(&f.authority, &f.invocation, &f.state).reason,
        Reason::TotalLimitExceeded
    );
}
...
fn stale_future_and_pregrant_invocations_are_rejected() {
    for created in [500, 4_000, 6_000] {
        let mut f = fixture();
        f.invocation.created = created;
        resign_invocation(&mut f);
        assert_eq!(
            border_check(&f.authority, &f.invocation, &f.state).reason,
            Reason::InvocationTimeInvalid
        );
    }
}
...
fn currency_resource_subject_capability_and_audience_are_bound() {
    ...
    let cases: Vec<(Mutation, Reason)> = vec![
        ...
        (
            |f| f.invocation.resource = "merchant/other".into(),
            Reason::ResourceMismatch,
        ),
        (
            |f| f.invocation.subject = "agent:rogue".into(),
            Reason::SubjectMismatch,
        ),
        (
            |f| f.invocation.capability = "payment.refund".into(),
            Reason::CapabilityMismatch,
        ),
        (
            |f| f.invocation.audience = "other-gateway".into(),
...
fn expiry_use_replay_revocation_and_offline_rules_hold() {
    ...
    let mut f = fixture();
    f.state.uses.insert(f.invocation.authority_id.clone(), 5);
    assert_eq!(
        border_check(&f.authority, &f.invocation, &f.state).reason,
        Reason::MaxUsesExceeded
    );
    let mut f = fixture();
    f.state.used_nonces.insert(f.invocation.nonce.clone());
    assert_eq!(
        border_check(&f.authority, &f.invocation, &f.state).reason,
        Reason::NonceReplay
    );
    ...
    f.state
        .revoked_ids
        .insert(f.authority.revocation.id.clone());
    assert_eq!(
        border_check(&f.authority, &f.invocation, &f.state).reason,
        Reason::Revoked
    );
    ...
    f.state.revocation_age_sec = 31;
    assert_eq!(
        border_check(&f.authority, &f.invocation, &f.state).reason,
        Reason::RevocationStale
    );
    let mut f = fixture();
    f.state.online = false;
    assert_eq!(
        border_check(&f.authority, &f.invocation, &f.state).reason,
        Reason::OfflineValueExceeded
    );
}
...
fn authorization_consumes_state_and_chains_receipts() {
    ...
    assert_eq!(
        authorize_and_record(&f.authority, &f.invocation, &mut f.state).reason,
        Reason::NonceReplay
    );
}
...
fn receipt_is_signed_and_gateway_key_is_pinned() {
    let f = fixture();
    let claims = border_check(&f.authority, &f.invocation, &f.state)
        .receipt
        .unwrap();
    ...
    trusted.insert(
        ...
        b64(SigningKey::from_bytes(&[56u8; 32])
            .verifying_key()
            .as_bytes()),
    );
    assert!(!verify_signed_receipt(&receipt, &trusted));
...
fn canonical_encoding_has_no_newline_boundary_collision() {
    let mut a = fixture().authority;
    let mut b = a.clone();
    a.issuer = "human\nagent".into();
    a.subject = "worker".into();
    b.issuer = "human".into();
    b.subject = "agent\nworker".into();
...
fn resource_traversal_and_false_prefixes_are_rejected() {
    ...
    assert!(!resource_within(
        "vault/public/../private/key",
        "vault/public"
    ));
}
...
fn delegation_pair() -> (AuthorityObject, AuthorityObject, SigningKey, SigningKey) {
    ...
    let mut parent = AuthorityObject {
        issuer: "human".into(),
        subject: "agent".into(),
        capability: "data.read".into(),
        resource: "vault/public/*".into(),
        scope: Scope {
            currency: None,
            max_per_action_minor: 0,
            max_total_minor: 0,
        },
        constraints: Constraints {
            not_before: 1_000,
        ...
        subject_pub: b64(agent.verifying_key().as_bytes()),
        sig: String::new(),
    };
    ...
    let mut child = AuthorityObject {
        ...
        subject: "worker".into(),
        capability: "data.read".into(),
        resource: "vault/public/report".into(),
        scope: Scope {
            currency: None,
            max_per_action_minor: 0,
            max_total_minor: 0,
        ...
        revocation: parent.revocation.clone(),
        issuer_pub: String::new(),
        subject_pub: b64(worker.verifying_key().as_bytes()),
        sig: String::new(),
    };
...
#[test]
fn strictly_narrower_delegation_is_valid() {
    let (p, c, _, _) = delegation_pair();
    assert!(check_attenuation(&c, &p).ok);
}
...
fn every_important_delegation_widening_is_rejected() {
    ...
    for mutate in mutations {
        let (parent, mut child, _, agent) = delegation_pair();
        mutate(&mut child);
        sign_authority(&mut child, &agent);
...
fn delegation_must_be_signed_by_parent_subject() {
    let (parent, mut child, root, _) = delegation_pair();
    sign_authority(&mut child, &root);
    assert!(!check_attenuation(&child, &parent).ok);
}
...
fn malformed_json_denies_without_panicking() {
    ...
    assert!(verdict.json.contains("MALFORMED_REQUEST"));
}
